<template>
  <div>
    <div container m-auto mt-6>
      <div v-for="(v,k) in msgList" :key="k" w-full sm:w-3xl m-auto>
        <div text-2xl font-bold sm:text-left m-5>{{ k }}</div>
        <div v-for="msg in v" :key="msg.id" m-5
          class=" bg-white p-8 rounded-md shadow-sm whitespace-pre-wrap text-left">
